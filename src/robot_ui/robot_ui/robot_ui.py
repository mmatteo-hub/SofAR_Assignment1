...
from test_commander.robot_navigator import BasicNavigator, TaskResult
...
class RobotUI(Node) :
    ...
    def __init__(self):
        ...
        self._reached_goal_pub = self.create_publisher(EmptyMsg, 'reached_goal', 10)
        # Subscriber for when the other robot reached the goal
        self.create_subscription(EmptyMsg, 'other_reached_goal', self.other_reached_goal, 10)
        # Service for temporarely stopping the robot
        self.create_service(SetBool, 'toggle_wait', self.request_toggle_wait)
    ...
    def follow_current_path_until_goal(self):
        ...
        # TODO The future is not working for some reason, so we use a timer
    ...
    def goal_check_callback(self):
        ...
        self._reached_goal_pub.publish(EmptyMsg())
